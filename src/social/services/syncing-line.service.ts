...
export class LineSyncService {
    ...
    async validateToken(accessToken: string): Promise<boolean> {
        try {
            ...
            this.logger.log(`✅ Token valid for user: ${userData.displayName} (${userData.userId})`);
            
            return true;
        } catch (error) {
            this.logger.error('❌ LINE token validation failed:', error.response?.data || error.message);
            return false;
        }
    }
    ...
    async getContacts(accessToken: string): Promise<ContactInfo[]> {
        try {
            ...
            if (!isValidToken) {
                throw new BadRequestException('LINE access token không hợp lệ hoặc đã hết hạn');
            }

            // Get user profile to verify token works
            const profile = await this.getProfile(accessToken);
            this.logger.log(`📋 LINE user profile: ${profile.displayName} (${profile.userId})`);
            ...
            this.logger.warn('⚠️ LINE API does not provide direct contact access, using mock data for development');
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                throw new BadRequestException('LINE contact sync requires special implementation. Please use alternative contact import methods.');
            }
    ...
    async getProfile(accessToken: string): Promise<LineProfileResponse> {
        try {
            ...
            this.logger.error('❌ Error fetching LINE profile:', error.response?.data || error.message);
    ...
    private handleLineError(error: any): never {
        ...
        if (error.response?.status === 403) {
            throw new BadRequestException('Không có quyền truy cập LINE API. Vui lòng kiểm tra quyền truy cập');
        }
        ...
        if (error.response?.status === 429) {
            throw new BadRequestException('LINE API rate limit exceeded. Vui lòng thử lại sau');
        }
