...
export class FacebookSyncService {
    ...
    async validateToken(accessToken: string): Promise<boolean> {
        ...
        this.logger.log(`✅ Token valid for user: ${userData.name} (${userData.email})`);
        
        return true;
        } catch (error) {
        this.logger.error('❌ Facebook token validation failed:', error.response?.data || error.message);
        return false;
        }
    }
   ...
   */
    async getContacts(accessToken: string): Promise<ContactInfo[]> {
        ...
        if (!isValidToken) {
            throw new BadRequestException('Facebook access token không hợp lệ hoặc đã hết hạn');
        }
        ...
        if (!hasUserFriendsPermission) {
            this.logger.warn('⚠️ user_friends permission not available, using mock data for development');
            this.logger.log(`🔑 Available permissions: ${permissions.join(', ')}`);

            // Return mock data for development
            if (process.env.NODE_ENV === 'development') {
                this.logger.log('🧪 Using mock data because user_friends permission not available');
                return this.getMockContacts();
            } else {
                throw new BadRequestException('Ứng dụng chưa được Facebook phê duyệt để truy cập danh sách bạn bè. Vui lòng sử dụng tính năng import contacts khác.');
            }
        }
        ...
        while (nextUrl && requestCount < maxRequests) {
            ...
            this.logger.log(`📄 Page ${requestCount}: Fetched ${friendsData.data.length} friends, ${contacts.length} with valid emails`);
            
            // Safety break to avoid too many contacts
            if (contacts.length > 5000) {
            this.logger.warn('⚠️ Reached maximum friends limit (5000), stopping...');
            break;
            }
        }

        this.logger.log(`✅ Successfully fetched ${contacts.length} Facebook friends with valid emails`);
        return contacts;
        ...
        if (error.response?.status === 403) {
            throw new BadRequestException('Không có quyền truy cập danh sách bạn bè Facebook. Vui lòng cấp quyền "user_friends"');
        }

        if (error.response?.status === 400) {
            const errorMessage = error.response?.data?.error?.message || 'Lỗi Facebook API';
            throw new BadRequestException(`Facebook API Error: ${errorMessage}`);
        }

        if (error.response?.status === 429) {
            throw new BadRequestException('Facebook API rate limit exceeded. Vui lòng thử lại sau');
        }
    ...
    async getProfile(accessToken: string): Promise<FacebookMeResponse> {
        try {
            ...
            this.logger.error('❌ Error fetching Facebook profile:', error.response?.data || error.message);
    ...
    async checkPermissions(accessToken: string): Promise<string[]> {
        try {
            ...
            this.logger.error('❌ Error checking Facebook permissions:', error.response?.data || error.message);
            return [];
        }
    }
     ...
     */
    async getBasicFriends(accessToken: string): Promise<{ id: string; name: string }[]> {
        try {
            const response = await firstValueFrom(
